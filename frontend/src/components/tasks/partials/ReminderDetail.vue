				...
				>
					...
					>
						<SimpleButton
							...
							:key="k"
							class="option-button"
							:class="{'currently-active': p.relative_period === modelValue?.relative_period && modelValue?.relative_to === p.relative_to}"
						...
						<SimpleButton
							class="option-button"
							:class="{'currently-active': typeof modelValue !== 'undefined' && modelValue?.relative_to !== '' && presets.find(p => p.relative_period === modelValue?.relative_period && modelValue?.relative_to === p.relative_to) === undefined}"
...
const props = withDefaults(defineProps<{
	...
	modelValue: () => ({relative_period: 0, relative_to: '', reminder: ''}),
...
const reminder = ref<ITaskReminder>({relative_period: 0, relative_to: '', reminder: ''})
...
watch(
	...
	(newReminder) => {
		...
		reminder.value = {relative_period: 0, relative_to: '', reminder: ''}
	},
	{immediate: true},
)
