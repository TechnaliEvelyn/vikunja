			...
			<template v-if="editEnabled && showCreate">
				...
				>
					...
					<CustomTransition name="fade">
						<span
							v-if="createRelation.isPending.value || deleteRelation.isPending.value"
						...
						<span
							v-else-if="!createRelation.isPending.value || deleteRelation.isPending.value && saved"
...
const query = ref('')
const taskQuery = useTasks(() => ({params: {q: query.value, sort_by: ['done']}}), () => query.value !== '')
...
function mapRelatedTasks(tasks: ITask[]) {
	return tasks.map(task => {
		...
		const project = projectList.projects[task.project_id ?? 0]
