...
const {updateBucket} = vi.hoisted(() => ({updateBucket: vi.fn()}))
...
vi.mock('@/client/generated', async importOriginal => ({...await importOriginal<object>(),
 projectViewBucketsTasksList: vi.fn(async () => ({data: {items: buckets}})),
...
describe('ProjectKanban', () => {
	beforeEach(() => { updateBucket.mockClear(); updateBucket.mockResolvedValue({data: {id: 2, position: 200}}) })
