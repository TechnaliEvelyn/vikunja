...
const taskQuery = useTasks(() => ({params: taskSearchParams.value ?? {}}), () => taskSearchParams.value !== null)
const foundTasks = computed(() => taskSearchParams.value ? taskQuery.tasks.value.map(task => ({...task, type: ACTION_TYPE.TASK})) : [])
