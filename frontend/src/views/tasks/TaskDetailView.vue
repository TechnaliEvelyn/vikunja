			...
			<div class="columns mbs-2">
				...
				>
					...
					<Reactions
						...
						:disabled="!canWrite"
						@update:modelValue="taskQuery.refetch()"
					/>
...
const task = computed(() => reactive({...createTaskDraft(klona(taskQuery.task.value)), max_permission: taskQuery.task.value?.max_permission}))
const due_dateInput = computed({get: () => parseDateOrNull(task.value.due_date), set: (date: Date | string | null) => { task.value.due_date = parseDateOrNull(date)?.toISOString() ?? '' } })
const start_dateInput = computed({get: () => parseDateOrNull(task.value.start_date), set: (date: Date | string | null) => { task.value.start_date = parseDateOrNull(date)?.toISOString() ?? '' } })
const end_dateInput = computed({get: () => parseDateOrNull(task.value.end_date), set: (date: Date | string | null) => { task.value.end_date = parseDateOrNull(date)?.toISOString() ?? '' } })
...
const canWrite = computed(() => (
	(task.value.max_permission ?? 0) > PERMISSIONS.READ
))
...
async function onAttachmentsUpdated() {
	...
	if (result.data) replaceTaskEverywhere(queryClient, {...result.data, attachments: result.data.attachments ?? []})
}
...
function setActiveFields() {
	...
	activeFields.repeatAfter = (task.value.repeat_after ?? 0) > 0 || task.value.repeat_mode !== TASK_REPEAT_MODES.REPEAT_MODE_DEFAULT
	activeFields.startDate = Boolean(parseDateOrNull(task.value.start_date))
}

watch(taskQuery.task, async (loaded, previous) => {
	if (!loaded) return
	taskColor.value = loaded.hex_color ?? ''
	if (loaded.id !== previous?.id) setActiveFields()
...
watch(taskQuery.error, cause => {
	if (cause && [403, 404].includes((cause as {status?: number}).status ?? 0)) {
...
async function saveTask(
	...
	await taskStore.update(currentTask)
	setActiveFields()
