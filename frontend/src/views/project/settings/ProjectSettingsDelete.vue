...
const taskQuery = useTasks(() => ({params: {filter: `project in ${projectIdsToDelete.value.join(',')}`, per_page: 1}}), () => projectIdsToDelete.value.length > 0)
const totalTasks = computed(() => taskQuery.data.value?.pages[0]?.total ?? 0)
